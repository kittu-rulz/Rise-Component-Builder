...
Each item card's whole `<section>` can be reordered by dragging, but it is only marked as a native HTML5 drag source (`draggable`) for the duration of a gesture that begins on its `.drag-handle` button; `js/editor.js` toggles the `draggable` property on `mousedown`/`mouseup`/`dragend` rather than leaving it permanently `true`. Leaving `draggable` on at all times would make the browser start tracking a potential native drag on any mousedown over the card — since `dragstart`'s `event.target` is always the draggable section itself, never the specific descendant a gesture began on, that tracking would compete with and interrupt interactive children (range sliders, text inputs) even though `dragstart` could still be cancelled after the fact. Toggling `draggable` itself, instead of only cancelling `dragstart`, keeps those controls free to handle their own mouse interaction.
