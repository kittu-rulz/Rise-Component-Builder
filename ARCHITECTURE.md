...
Each item card's whole `<section>` is a native HTML5 drag source (`draggable="true"`) so the card can be reordered by dragging, but `dragstart`'s `event.target` is always that draggable section itself, never the specific descendant a gesture began on. `js/editor.js` tracks the real mousedown origin separately and only allows the native drag to proceed when it started on the `.drag-handle` button; otherwise `dragstart` is cancelled so range sliders, text inputs, and other interactive controls inside the card keep their own mouse interaction instead of being hijacked into a card drag.
