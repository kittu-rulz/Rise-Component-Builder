...
Playwright starts `tests/e2e/server.mjs` against the local workspace and drives Chromium without requiring a hosted environment. Browser suites cover the shell, schema editor, iframe preview, five modular interactions, persistence, downloads, responsive sizes, and selected accessibility checks, including builder modal focus trapping and trigger-focus restoration. axe-core inspects stable builder regions; iframe semantics also receive direct role, state, focus, and reference assertions. GitHub Actions runs both layers and retains failure traces, screenshots, videos, and coverage reports.
