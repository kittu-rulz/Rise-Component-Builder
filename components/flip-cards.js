...
export function generateHTML(config) {
  ...
  return `<div class="flip-cards-grid">${cards.map((card, index) => {
    ...
    <div class="flip-card" role="button" tabindex="0" aria-expanded="false" aria-controls="flip-card-back-${index}" aria-label="${escapeAttribute(card.front.title || 'Flip card')}: reveal back">
      ...
      <div class="flip-card-front" id="flip-card-front-${index}" aria-hidden="false"><div class="card-icon-badge">${frontArtwork}</div><h3>${escapeHTML(card.front.title || 'Front Title')}</h3><p>${sanitizeRichText(card.front.content || 'Click to reveal definition.')}</p></div>
      <div class="flip-card-back" id="flip-card-back-${index}" aria-hidden="true">${backArtwork ? `<div class="card-icon-badge">${backArtwork}</div>` : ''}<h3>${escapeHTML(card.back.title || 'Back Title')}</h3><p>${sanitizeRichText(card.back.content || 'Back description content goes here.')}</p></div>
    </div></div>`;
  }).join('')}</div>`;
}
export function generateCSS() { return '.flip-card{perspective:1000px}.flip-card-inner{transform-style:preserve-3d}.flip-card.flipped .flip-card-inner{transform:rotateY(180deg)}.card-icon-badge .custom-item-icon{width:32px;height:32px;object-fit:contain}'; }
